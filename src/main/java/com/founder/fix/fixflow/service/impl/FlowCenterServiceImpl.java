...
//				instances.put("icon", "icon/"+userId+"_small.png");
				if(StringUtil.isNotEmpty(userId)){
					...
					if(user!=null){
						...
						instances.put("userRealName", user.getPropertyValue("name"));
					}else{
	...
	public Map<String,Object> queryTaskParticipants(Map<String,Object> filter) throws SQLException {
		filter.put("processType", "queryPart");
	...
	public Map<String,Object> queryTaskInitiator(Map<String,Object> filter) throws SQLException {
		...
		try{
			...
			for(ProcessInstance tmp: processInstances){
				...
				persistentState.put("formUri",engine.getModelService().getDefaultFromUri(processDefinitionId));
				persistentState.put("taskInstanceId","");
				...
				if(user !=null){
					...
					persistentState.put("startAuthorRealName", user.getPropertyValue("name"));
				}else{
			...
			try{
				...
				for(TaskInstance tmp:instances){
					...
					if(StringUtil.isNotEmpty(assigneeUserId)){
						...
						if(tmpUser!=null){
							instanceMap.put("assgneeUserName", tmpUser.getPropertyValue("name"));
						}
					}else{
	...
	public Map<String, Object> getRollbackTask(Map<String, Object> params) throws SQLException {
		...
		try{
			...
			for(TaskInstance task :taskInstances){
				...
				if(user !=null){
					taskMap.put("assigneeUserName", user.getPropertyValue("name"));
				}
